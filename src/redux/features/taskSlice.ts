...
export const TaskSlice = createSlice({
  ...
  reducers: {
    addTask: (state, action: PayloadAction<PayloadActionTask>) => {
      const task = {
        id: state.tasks.length
          ? String(Number(state.tasks[state.tasks.length - 1].id) + 1)
          : "1",
        ...
        order: {
          column: 1,
          row: state.tasks.length
            ? state.tasks.filter((item) => item.order.column === 1).length + 1
            : 1,
        },
      };
    ...
    reOrder: (state, action: PayloadAction<DropResult>) => {
      ...
      if (isSameColumn) {
        const columnItems = [
          ...state.tasks
            .filter(
              (item) =>
                item.order.column.toString() ===
                action.payload.source.droppableId
            )
            .sort((a, b) => a.order.row - b.order.row),
        ];
        const [removed] = columnItems?.splice(
      ...
      const remainingTasks = (destination: boolean) =>
        state.tasks.filter(
          (item) =>
            item.order.column.toString() !==
            action.payload[destination ? "destination" : "source"]?.droppableId
        );
      const destinationColumnItems = (destination: boolean) =>
        state.tasks.filter(
          (item) =>
            item.order.column.toString() ===
            action.payload[destination ? "destination" : "source"]?.droppableId
        );
      const changedDestinationColumnItems = destinationColumnItems(true).map(
        (item) => {
      ...
      state.tasks = [
        ...
        ...destinationColumnItems(false).map((item) => {
    ...
    filtering: (state, { payload }: { payload: number }) => {
      if (payload === 1) {
        state.tasks = state.tasks.sort((a, b) =>
          dayjs(a.createdDate).isBefore(b.createdDate) ? 1 : -1
        );
        return;
      }
      if (payload === 2) {
      ...
      if (payload === 6) {
        state.tasks = state.tasks.sort(
          (a, b) => Number(b.storyPoints) - Number(a.storyPoints)
        );
        return;
      }
    },
...
export const { addTask, reOrder, emptyBoard, filtering } = TaskSlice.actions;
